...
class ReplicaXRDatasetConfig(object):
    def __init__(self):
        ...
        self.type2class = {'basket': 0, 'bed': 1, 'cabinet': 2, 'chair': 3, 'sofa': 4, 'table': 5,
                           ...
                           'sink': 31, 'lamp': 32, 'bathtub': 33, 'bag': 34, 'stool': 35}
...
def transform_and_save_mesh(input_filepath, save_filepath, trans_vector, rot_vec=None):
...
def transform_and_save_infosemanticjson(object_semantic_filepath, saved_object_semantic_filepath, gravity_center):
    ...
    for obj_bbox in v_obj_bbox:
        ...
        obb_R = R.from_quat(obj_bbox['oriented_bbox']
...
def transform_and_save_bbox_prior(object_semantic_filepath, saved_3dbbox_prior_filepath, gravity_center, scene_name=None):
    ...
    for obj_bbox in v_obj_bbox:
        ...
        obb_T[:3, :3] = obb_R
        obb_T[:3, 3] = obb_t
        angles = R.from_quat(obj_bbox['oriented_bbox']['orientation']['rotation']).as_euler(
            'zyx', degrees=True)
...
def main(dataset_path):
    ...
    for folder in scene_folders:
        ...
        cam_position_filepath = osp.join(
            dataset_path, folder, scene_name + '_6dof.txt')
        ...
        assert osp.exists(
            cam_position_filepath), f"{cam_position_filepath} doesnt exist..."
        assert osp.exists(
        ...
        if not b_skip_mesh:
            transform_and_save_mesh(
                scene_mesh_filepath, saved_mesh_filepath, gravity_center)
